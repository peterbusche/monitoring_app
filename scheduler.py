...
status_counter = defaultdict(int) # use this for clarity, so we can track which endpoint sent the 500+ error
...
def check_endpoints(app_config):
    ...
    for e in endpoints:
        try:
            ...
            db.add(record) # does this use a transaction? yes, instantiation of db = SessionLocal() will implicily start a new transaction
...
def cleanup_old_data(app_config):
    ...
    deleted = db.query(EndpointStatus).filter(EndpointStatus.checked_at < cutoff_time).delete()
    db.commit()
...
def start_scheduler(app_config):
    ...
    scheduler.add_job(
        ...
        replace_existing=True)          # if another job with this id runs over, replace it with this job
